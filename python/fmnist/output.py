...
class Synapse:
    ...
    def AddCompStrengths(self, strs):
        self.comp_strs.append(strs)
...
class Output:
    ...
    def LoadNeuron(self, lid, nid, last=True):
        # # Load all neuron data

        nfn = f"NEURON_{lid}_{nid}"
    ...
    def LoadSynapse(self, lid, nid, sid, last=True):

        sfn = f"SYNAPSE_{lid}_{nid}_{sid}"
        ...
        with open(self.path+sfn, 'r') as f:
            ...
            while ids:
                ...
                err_data = f.readline()
                pid_data = f.readline()
                cid_data_pre = f.readline()
                comp_data = f.readline()
                comp_str = f.readline()


                if not last:

                    time_data = time_data.strip().split()[1:]
                    lat_data = lat_data.strip().split()[1:]
                    lon_data = lon_data.strip().split()[1:]
                    rad_data = rad_data.strip().split()[1:]
                    str_data = str_data.strip().split()[1:]
                    spk_data = spk_data.strip().split()[1:]
                    in_data = in_data.strip().split()[1:]
                    err_data = err_data.strip().split()[1:]
                    pid_data = pid_data.strip().split()[1:]
                    cid_data_pre = cid_data_pre.strip().split()[1:]
                    comp_data = comp_data.strip().split()[1:]
                    comp_str = comp_str.strip().split()[1:]
                    ...
                    synapse.AddCompStrengths(comp_str)
