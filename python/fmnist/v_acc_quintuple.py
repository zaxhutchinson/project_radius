...
INCORR_RATE = 5
labels = [0,5,1,3,8]
target_rates = {3:10, 5:5, 1:7.5, 0:12.5, 8:15}
SIZE = len(labels)
BATCH=10
EPOCH=SIZE*BATCH
...
# print(LENGTH//EPOCH)
for e in range(1000):#LENGTH//EPOCH):
    ...
    for i in range(S,E):
        ...
        for l in labels:
            val = abs(1-(num_spikes / target_rates[l]))
            # val = abs(target_rates[l]-num_spikes)
            if val < dist:
                dist = val
                label = l
...
plt.figure()
for l in labels:
    plt.plot(spikes_by_label_per_epoch[l], label=str(l))

    print(f"{l} {np.mean(spikes_by_label_per_epoch[l][-200:])}")
    # print(f"STD 400-500 {l} {np.std(spikes_by_label_per_epoch[l][400:500])}")
    # print(f"MEAN 400-500 {l} {np.mean(spikes_by_label_per_epoch[l][400:500])}")
    # print(f"STD 500-600 {l} {np.std(spikes_by_label_per_epoch[l][500:600])}")
    # print(f"MEAN 500-600 {l} {np.mean(spikes_by_label_per_epoch[l][500:600])}")
plt.legend()
plt.xlabel("Epoch")
...
print(f"STD {np.std(correct[-200:])}")
print(f"MEAN {np.mean(correct[-200:])}")
